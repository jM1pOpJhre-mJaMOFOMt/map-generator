<template>
  ...
  <div class="overlay top right flex-col gap">
    <div v-if="!state.started" class="settings">
      ...
      <div class="adjustHeading" title="0° will point directly towards the road.">
        ...
        <div v-if="settings.adjustHeading" class="indent">
          <label class="flex wrap">
            Deviation
            <input type="range" v-model.number="settings.headingDeviation" min="0" max="50" />
      ...
      Radius<input type="number" v-model.number="settings.radius" @change="handleRadiusInput"/>m
    </div>
    <hr />
  ...
  <Button v-if="canBeStarted" @click="handleClickStart" :class="state.started ? 'bg-danger' : 'bg-success'" :text="state.started ? 'Pause' : 'Start'" title="Space bar/Enter" />
...
// Process
document.onkeydown = () => {
  if (window.event.keyCode == "13" || window.event.keyCode == "32")  handleClickStart();
};
